...
public class TextFieldController : MonoBehaviour
{
  ...
  {
    if ((this.force_pixel_font || PlayerPrefs.GetString("font", "pixel") == "pixel") && !this.force_smooth_font)
    {
      this.smod = 1f;
      this.pmod = 1;
      this.pmod2 = 1;
      this.sprites = Resources.LoadAll<Sprite>("Bitmaps/Misc/pixelFont");
    }
    else
    {
  ...
  private void Update()
  {
    ...
    Vector3 worldPoint = Camera.main.ScreenToWorldPoint(new Vector3(screen1.x, screen1.y, 0.0f));
    if ((double) this.container.transform.position.x >= (double) worldPoint.x)
      this.container.transform.position = new Vector3(worldPoint.x, this.container.transform.position.y, this.container.transform.position.z);
    if ((double) this.container.transform.position.y >= -(double) worldPoint.y && this.blockTop && (double) this.directionY == -1.0)
      this.container.transform.position = new Vector3(this.container.transform.position.x, -worldPoint.y, this.container.transform.position.z);
    ...
    worldPoint = Camera.main.ScreenToWorldPoint(new Vector3(screen2.x, screen2.y, 0.0f));
    if ((double) this.container.transform.position.x > (double) worldPoint.x)
      return;
    this.container.transform.position = new Vector3(worldPoint.x, this.container.transform.position.y, this.container.transform.position.z);
  }
  ...
  private void addText(string text)
  {
    for (int index1 = 0; index1 < text.Length; ++index1)
    {
      ...
      this.bytes = !text[index1].ToString().Equals("^") ? (!text[index1].ToString().Equals("#") ? Encoding.UTF8.GetBytes(text[index1].ToString()) : Encoding.UTF8.GetBytes("@")) : Encoding.UTF8.GetBytes(" ");
