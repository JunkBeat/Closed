...
public class TravelAgency : ObjectActionController
{
  ...
  public override void mouseOver()
  {
    ...
    int index1 = 0;
    int inputText = 9999999;
    for (int index2 = 0; index2 < this.routes.Count; ++index2)
    {
      int num = 0;
      for (int index3 = 0; index3 < this.routes[index2].Count - 1; ++index3)
        num += this.routes[index2][index3].travelTimes[this.routes[index2][index3].nodes.IndexOf(this.routes[index2][index3 + 1])];
      if (num < inputText)
      {
        inputText = num;
        index1 = index2;
    ...
    {
      bool red = false;
      if (GameDataController.gd.getObjective("car_travel") && inputText > this.fc.checkFuel())
      {
        red = true;
        r = 1f;
        g = 0.0f;
    ...
    if (GameDataController.gd.getObjective("car_travel"))
    {
      ...
      if (inputText != 0)
        inputText = inputText / 5 + 1;
      else
        this.fc.hideFuelMissingMeter();
    }
    string prefixedShort = GameStrings.getPrefixedShort(this.dkvs, this.objectName, true);
    ...
    string str = strArray[inputText % 10 <= 4 || inputText % 10 >= 20 ? numArray[inputText % 10 < 5 ? inputText % 10 : 5] : 2];
    if (TravelAgency.IsThisException(inputText))
      str = strArray[2];
    string text = prefixedShort + ". ^" + GameStrings.getString(GameStrings.gui, key) + " " + (object) inputText + " " + str + ".";
    if (GameDataController.gd.getObjective("car_travel"))
    {
      float num = Mathf.Round(100f * this.fc.getPercent()) - Mathf.Round(100f * this.fc.getPercent(this.fuel));
      text = text + " ^" + GameStrings.getString(GameStrings.gui, "fuel_used") + " " + (object) num + "% " + GameStrings.getString(GameStrings.gui, "fuel_used2") + ".";
